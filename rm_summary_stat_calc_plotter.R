...
calculate_min <- function(results) {
  ...
  rm_range_min <- apply(rm_range_array, c(1, 2), function(x) {
    if(all(is.na(x) | is.infinite(x))) {
      return(NA)  # Return NA if all values are NA or Inf
    } else {
...
calculate_sd <- function(results) {
  ...
  rm_range_sd <- apply(rm_range_array, c(1, 2), function(x) {
    if(all(is.na(x) | is.infinite(x))) {
      ...
      valid_x <- x[!is.na(x) & !is.infinite(x)]
      if(length(valid_x) <= 1) return(NA)  # Need at least 2 values for SD
      return(sd(valid_x))
    }
  })
...
create_summary_stat_plots_with_histograms <- function(results, summary_stats) {
  ...
  if (!dir.exists("figures/summary_stats_figures")) {
    dir.create("figures/summary_stats_figures", recursive = TRUE)
  }
  ...
  for (stat_name in names(summary_stats)) {
    # Skip if the statistic doesn't follow our expected pattern
    if (!grepl("_rm_range$|_difference$|_bias_factor$", stat_name)) {
      next
    }
    
    # Determine the base statistic type (min, q1, median, etc.)
    base_stat_type <- gsub("_rm_range$|_difference$|_bias_factor$", "", stat_name)
    
    # Prepare a title-friendly version
    ...
    output_filename <- paste0("figures/summary_stats_figures/", base_stat_type, "_summary_stats.jpg")
