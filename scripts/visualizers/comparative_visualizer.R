...
comparative_visualizer <- function(input_matrix, title = "Northeast US Precipitation", 
  ...
  # If min_val and max_val are provided, use them for the color scale
  if (!is.null(min_val) && !is.null(max_val)) {
    terra::plot(r, 
                ...
                ylab = "Latitude",
                legend = TRUE,
                range = c(min_val, max_val),  # Set the range for the legend
                plg = list(title = "mm/day"),
                axes = TRUE)
  } else {
    # Default behavior when min_val and max_val are not provided
    terra::plot(r, 
                main = title,
                col = rev(hcl.colors(100, "Blues")),
                xlab = "Longitude",
