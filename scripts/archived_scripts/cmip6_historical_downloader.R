...
# downloader function takes a model name and aoi sf as input
# loops through model vector
# uses model names to create dirs if none are found in the output dir
# includes user prompting
downloader <- function(models, area_of_interest) {
  for (model in models) {
    # this implements the automatic directory creation
    model_dir <- format_model_dir(model)
    outdir <- file.path("data/gcm_ne_subset", model_dir, "historical")
    ...
    for(i in seq_along(historical_year_lookup)) {
      ...
      tryCatch({
        ...
        cat(sprintf("  Error downloading %s - %s - Year %d: %s\n", 
                    model, "historical", year_val, e$message))
      })
    }
    ...
    # prompt for continuation to next model
    if (!prompt_continue()) {
