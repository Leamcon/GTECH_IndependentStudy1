...
subset_netcdf_with_shp <- function(input_nc, output_nc, shapefile_path) {
  ...
  # read and validation shape, inc. error handling
  ...
  # set crs to wgs84
  boundary_sf_transformed <- st_transform(boundary_sf, crs = 4326)
  ...
  # implementing a chunk process system
  # data is operated by chunks to reduce chance of issues
  chunk_size <- 12
