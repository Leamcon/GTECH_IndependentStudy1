...
lines(ma(prcp_catskills[5,1,],10) ~ yr, col="red")
...
lines(ma(prcp_catskills_region,10) ~ yr, col="red")
...
###########################
# SECTION TO LOOK AT MOVING AVERAGE


# # THIS DID NOT WORK
# # 10 yr moving average func
# ma10 <- function(x) { as.vector(stats::filter(x, rep(1 / 10, 10), sides = 1)) }
# prcp_ma <- apply(cpc_annual_means_ne_subset,c(1,2),ma10)

# SO I WAS FORCED TO DO IT WITH LOOPS
# mask out non-data (missing) grid points
duration <- 10 # how many yrs moving average.
